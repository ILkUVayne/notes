...
1. 本地队列为空时，尝试从全局队列中获取转移一批g到本地队列，最多转移32个
2. 从本地队列获取g，检查栈信息，并做相应设置后，返回g
...
当goroutine执行完毕，调度器相关函数会将g放回p队列，实现复用

主要逻辑：

1. 优先放入本地空闲队列
2. 本地队列长度大于64时，转移部分到全局列表，本地队列长度小于32时，停止转移
...
2. 如果next==false,尝试优先将g放入pp.runq本地可运行队列
